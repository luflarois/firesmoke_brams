...
!if(stepDebug) print *,mynum,SFCLYR_JULES'
  ...
  if (CCATT==1 .and. chemistry >= 0) then
     ...
     !
!if (mchnum==master_num) print *,'LFR-DBG - Chamando sources_driver = timestep ',time
     ...
     !print *,'LFR-DBG Size of plume_fre_g = ',size(plume_fre_g,1),size(plume_fre_g,2)
     !print *,'LFR-DBG- Size of plume_fre_g(iflam_frac) : ',size(plume_fre_g(iflam_frac,1)%pvar,1),size(plume_fre_g(iflam_frac,1)%pvar,2)
     !print *,'LFR-DBG- Size of plume_fre_g(imean_frp ) : ',size(plume_fre_g(imean_frp ,1)%pvar,1),size(plume_fre_g(imean_frp ,1)%pvar,2)
     !print *,'LFR-DBG- Size of plume_fre_g(istd_frp  ) : ',size(plume_fre_g(istd_frp  ,1)%pvar,1),size(plume_fre_g(istd_frp  ,1)%pvar,2)
