               ...
               do k = k1, k2
                  ...
                  if (aerosol == 1) then
                     ...
                     do imode = 1, nmodes
                        ...
                        if (plume_fre_g(imean_frp )%pvar(i,j)>0.0) then
                           write (88+mynum,fmt=('(2(I3.3,1X),10(E15.6,1X),2(I2.2,1X))')) i,j &
                                     ,   plume_fre_g(iflam_frac)%pvar(i,j) &
                                     ,   plume_fre_g(imean_frp )%pvar(i,j) &
                                     ,   plume_fre_g(istd_frp  )%pvar(i,j) &
                                     ,   plume_fre_g(imean_size)%pvar(i,j) &
                                     ,   plume_fre_g(istd_size )%pvar(i,j) &
                                     ,   burnt_area &
                                     ,   FRP &
                                     ,   convert_smold_to_flam &
                                     ,   q_smold_kgm2 &
