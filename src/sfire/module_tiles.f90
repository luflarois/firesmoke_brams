...
MODULE module_tiles
  ...
  INTERFACE set_tiles
    MODULE PROCEDURE set_tiles1 , set_tiles2, set_tiles3 !, set_tiles_once !COMENTADO POR ISILDA CUNHA MENEZES
  END INTERFACE
...
!     USE module_domain_type, ONLY : domain, MAX_TILING_ZONES
!     IMPLICIT NONE
!     TYPE(domain)                   , INTENT(INOUT)  :: grid
...
!  END SUBROUTINE set_tiles_once
  ...
  SUBROUTINE set_tiles2 ( grid , ids , ide , jds , jde , ips , ipe , jps , jpe )
     ...
     IF( ips < smx )THEN;WRITE(mess,'(3A4)')'ips','<','smx';ENDIF !CALL WRF_ERROR_FATAL(mess);ENDIF
     IF( ipe > emx )THEN;WRITE(mess,'(3A4)')'ipe','>','emx';ENDIF !CALL WRF_ERROR_FATAL(mess);ENDIF
     IF( jps < smy )THEN;WRITE(mess,'(3A4)')'jps','<','smy';ENDIF !CALL WRF_ERROR_FATAL(mess);ENDIF
     IF( jpe > emy )THEN;WRITE(mess,'(3A4)')'jpe','>','emy';ENDIF !CALL WRF_ERROR_FATAL(mess);ENDIF
