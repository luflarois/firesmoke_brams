...
!  real ::temp_dnp(nnzp(1),nnxp(1),nnyp(1))
  ...
  logical, save :: first_combinit_call = .TRUE. ! PARA LER COMBUSTIVEL UMA VEZ
...
!  integer, intent(in) :: ngra 
  ...
  integer :: id
  integer :: tmp
  integer, parameter :: nlinhas = 24286
  integer,parameter :: ncolun = 16439
  ...
  real :: ae,res_ant, R, point_latitude, point_longitude
  real :: grid_longitude, grid_latitude,distance
  !integer,dimension(nlinhas,ncolun) :: mar
  integer, allocatable :: mar(:,:)
  integer,dimension(nnxp(1),nnyp(1)) :: tmar 
  allocate(tcomb(nnxp(1),nnyp(1),14))
  allocate(idclass(nlinhas,ncolun))
  allocate(mar(nlinhas,ncolun))
...
!*****Vai calcular a posicao cartesiana polar do sistema geometrico dos combustiveis a serem interpolados*******
  ...
  !print*,"sfire -estou na rotina dos combustiveis - ver se o ficheiro existe"
    ...
    tcomb(:,:,:)=0.
    !print*,"sfire -estou na rotina dos combustiveis - vou ler o ficheiro"
...
!******Vai achar a posicao do no na malha cartesiana polar*********
    !   print*,"Vai achar a posicao do no na malha cartesiana polar"
     ...
     !print*,"sfire -estou na rotina dos combustiveis - vou calcular o ponto proximo"

       if((lat >= temp_glat(1,1).and.lat <= temp_glat(1,nnyp(1))).and.(lon >= temp_glon(1,1))&
        .and.(lon <= temp_glon(nnxp(1),1)))then
         ...
         do j = 1,nnyp(1)
	      do l = 1,nnxp(1)
	        ...
	        grid_longitude = temp_glon(l,j)
                distance = R*acos(sin(point_latitude*DEGRAD)*sin(grid_latitude*DEGRAD)&
